...
def generate_tokens(user_id: int, phone_number: str) -> str:
    claims = {'sub': phone_number, 'user_id': user_id}
...
def get_phone_by_token(token: str) -> str:
    try:
        claims = jwt.get_unverified_claims(token)
        return claims['sub']
...
def check_profile_view_permission(token: str, phone: str) -> bool:
    viewer_phone = get_phone_by_token(token)
    if viewer_phone == phone:
        return True
    return False
